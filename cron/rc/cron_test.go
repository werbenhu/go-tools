...
func (d DummyJob) Run(id EntryID) {
	panic("YOLO")
}
...
func TestStopCausesJobsToNotRun(t *testing.T) {
	...
	cron.Start()
	cron.Stop()
	cron.AddFunc("* * * * * ?", func(id EntryID) { wg.Done() })
...
func TestAddWhileRunningWithDelay(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(id EntryID) { atomic.AddInt64(&calls, 1) })
...
func TestRemoveBeforeRunning(t *testing.T) {
	...
	cron := newWithSeconds()
	id, _ := cron.AddFunc("* * * * * ?", func(id EntryID) { wg.Done() })
	cron.Remove(id)
	cron.Start()
	defer cron.Stop()
...
func TestSnapshotEntries(t *testing.T) {
	...
	cron := New()
	cron.AddFunc("@every 2s", func(id EntryID) { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestMultipleEntries(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("0 0 0 1 1 ?", func(id EntryID) {})
	cron.AddFunc("* * * * * ?", func(id EntryID) { wg.Done() })
	id1, _ := cron.AddFunc("* * * * * ?", func(id EntryID) { t.Fatal() })
	id2, _ := cron.AddFunc("* * * * * ?", func(id EntryID) { t.Fatal() })
	cron.AddFunc("0 0 0 31 12 ?", func(id EntryID) {})
	cron.AddFunc("* * * * * ?", func(id EntryID) { wg.Done() })
...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	cron.AddFunc("0 0 0 31 12 ?", func(id EntryID) {})
	cron.AddFunc("* * * * * ?", func(id EntryID) { wg.Done() })
	cron.Schedule(Every(time.Minute), FuncJob(func(id EntryID) {}))
	cron.Schedule(Every(time.Second), FuncJob(func(id EntryID) { wg.Done() }))
	cron.Schedule(Every(time.Hour), FuncJob(func(id EntryID) {}))
...
func TestLocalTimezone(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc(spec, func(id EntryID) { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	cron := newWithSeconds()
	hourJob := cron.Schedule(Every(time.Hour), FuncJob(func(id EntryID) {}))
	cron.Schedule(Every(time.Second), FuncJob(func(id EntryID) {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(id EntryID) { atomic.AddInt64(&calls, 1) })
	cron.Schedule(new(ZeroSchedule), FuncJob(func(id EntryID) { t.Error("expected zero task will not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		cron.AddFunc("* * * * * *", func(id EntryID) {})
		cron.Start()
		cron.AddFunc("* * * * * *", func(id EntryID) { time.Sleep(2 * time.Second) })
		cron.AddFunc("* * * * * *", func(id EntryID) {})
		time.Sleep(time.Second)
