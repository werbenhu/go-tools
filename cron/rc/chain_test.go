...
func appendingJob(slice *[]int, value int) Job {
	var m sync.Mutex
	return FuncJob(func(id EntryID) {
		m.Lock()
...
func appendingWrapper(slice *[]int, value int) JobWrapper {
	return func(j Job) Job {
		return FuncJob(func(id EntryID) {
			appendingJob(slice, value).Run(id)
			j.Run(id)
		})
	}
}
...
func TestChain(t *testing.T) {
	...
	NewChain(append1, append2, append3).Then(append4).Run(123)
...
func TestChainRecover(t *testing.T) {
	panickingJob := FuncJob(func(id EntryID) {
...
func (j *countJob) Run(id EntryID) {
	j.id = id
	j.m.Lock()
	j.started++
	j.m.Unlock()
...
func TestChainSkipIfStillRunning(t *testing.T) {
	...
	t.Run("different jobs independent", func(t *testing.T) {
		...
		for i := 0; i < 11; i++ {
			go wrappedJob1.Run(j1.id)
			go wrappedJob2.Run(j2.id)
		}
