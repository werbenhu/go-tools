...
//
// Parser is faster for obtaining multiple fields from JSON.
func GetBytes(data []byte, key string) []byte {
	p := handyPool.Get()
	v, err := p.ParseBytes(data)
	if err != nil {
		handyPool.Put(p)
...
//
// Parser is faster for obtaining multiple fields from JSON.
func GetFloat64(data []byte, key string) float64 {
	p := handyPool.Get()
	v, err := p.ParseBytes(data)
	if err != nil {
		handyPool.Put(p)
...
//
// Parser is faster for obtaining multiple fields from JSON.
func GetBool(data []byte, key string) bool {
	p := handyPool.Get()
	v, err := p.ParseBytes(data)
	if err != nil {
		handyPool.Put(p)
...
func Exists(data []byte, key string) bool {
	p := handyPool.Get()
	v, err := p.ParseBytes(data)
	if err != nil {
		handyPool.Put(p)
